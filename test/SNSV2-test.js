...
describe('SNSV2', function () {
  ...
  before(async function () {
    ...
    //造数据
    const addr = await this.linkKey.owner()
    console.log('ownerAddress----', await this.linkKey.owner())
    const price = await this.sns.getPrice()
    await this.sns.mint('peifeng', { value: price })

    expect(await this.sns.getResolverOwner('peifeng.key')).to.equal(addr)

    this.SNSV2 = await ethers.getContractFactory('SNSV2')

    this.snsV2 = await upgrades.upgradeProxy(this.sns.address, this.SNSV2)

    console.log('snsV2----', this.snsV2.address)
  })
  ...
  //test setparam
  it('[1]SNSV2:Test setparam ', async function () {
    const balance = await this.linkKey.balanceOf(
      '0xB3eF1C9718F3EAFaeb6fd7Ac63E8f43493101Ded',
    )

    console.log('balance----', balance)

    await this.snsV2.setCoins(1, this.linkKey.address)
    await this.snsV2.setCoinsPrice(1, 1500)

    await this.snsV2.setFeeTo('0x185BA2eD90fA734c34abd6F2283f22C785afba75')
  })
