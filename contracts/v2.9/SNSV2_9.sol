...
contract SNSV2_9 is NFTV2 , ISns{
     ...
     * @param coinsType_ 0 matic 1key 2lowb 3ustc
     */
    function mint(string memory name_,uint256 coinsType_,address inviter_) external payable {
        ...
        (uint256 maticPrice,uint256 coinsPrices,uint256 lowbPrices,uint256 usdcPrices) = getPrice(inviter_);
        ...
        if(coinsType_ == 0){
            ...
            if(_invite.canInviter(inviter_)) {
                uint256 inviterIncome = maticPrice * (100 - _invite.inviteDiscountRate()) / 100;
                ...
                (success, ) = payable(_feeTo).call{value: maticPrice - inviterIncome}("");
            } else {
                (success, ) = payable(_feeTo).call{value: maticPrice}("");
            }
        } else{
            uint256 prices;
     ...
     */
    function shortNameMint(string memory name_, uint256 payWay_,address inviter_) external virtual payable {
        name_ = name_.trim(" ");
        ...
        if(priceOfShort.maticPrice == 0 && priceOfShort.keyPrice == 0){
            ...
            if(payWay_ == 1){
                prices = priceOfShort.maticPrice;
                uint256 inviterIncome = prices * (100 - _invite.inviteDiscountRate()) / 100;  
                ...
                prices = priceOfShort.keyPrice;
                uint256 inviterIncome = prices * (100 - _invite.inviteDiscountRate()) / 100;  
                if(_invite.canInviter(inviter_)) {
                    IERC20(priceOfShort.keyAddress).transferFrom(_msgSender(),inviter_,inviterIncome);
                    _invite.setInviterIncome(inviter_,0,inviterIncome);
                    success = IERC20(priceOfShort.keyAddress).transferFrom(_msgSender(),address(_feeTo),prices - inviterIncome);
                }else{
                    success = IERC20(priceOfShort.keyAddress).transferFrom(_msgSender(),address(_feeTo),prices);
                }
            }else{
