...
contract SNSV3_5 is NFTV2 , ISns{
     ...
     * @param coinsType_ 0 matic 1key 2lowb 3ustc // shortMint  0:matic 1:key 3:usdc
     */
    function mint(string memory name_,uint256 coinsType_,address inviter_,bytes32[] calldata merkleProof_) external payable {
        ...
        PriceInfo memory priceInfo = getPrice(_msgSender(),name_,inviter_,merkleProof_);
        ...
        if (success) {
            whiteListInfo.isMint[_msgSender()] = true;
        }else{
            uint256 inviterIncome;
            ...
            if(coinsType_ == 0){
                ...
                if(_invite.canInviter(inviter_)) {
                    inviterIncome = price * (100 - _invite.inviteDiscountRate()) / 100;
                    ...
                    (success, ) = payable(feeTo).call{value: price - inviterIncome}("");
                } else {
                    (success, ) = payable(feeTo).call{value: price}("");
                }
            } else{
                if(coinsType_ == 1){
                ...
                require(price != 2**256 -1,"price error!!!");
    ...
    * @dev NFT transfer
     ...
     * @dev getPrice
     */
    function getPrice(address minter_,string memory name_,address inviter_,bytes32[] calldata merkleProof_) public view override returns (PriceInfo memory priceInfo) {
    ...
    *loser foundation address: 0x1EC0E4DC543566f26B73800700080B4b2f3fD208
    ...
    WhiteListInfo private whiteListInfo;

    function setWhiteListInfo(bytes32 threeMerkleRoot_,bytes32 fourSevenMerkleRoot_,bytes32 eightPlusMerkleRoot_) public onlyOwner{
        whiteListInfo.threeMerkleRoot = threeMerkleRoot_;
        whiteListInfo.fourSevenMerkleRoot = fourSevenMerkleRoot_;
        whiteListInfo.eightPlusMerkleRoot = eightPlusMerkleRoot_;
    }

    function isMerk(address addr_,bytes32 root_,bytes32[] calldata merkleProof_) public view returns(bool){
        if(!whiteListInfo.isMint[addr_]){
