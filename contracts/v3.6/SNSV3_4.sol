...
//      * @param coinsType_ 0:matic 1:key 2:lowb 3:ustc  4:matic and key 
//      * shortMint  0:matic 1:key 
//      */
...
//         uint256 price = 0;
//         uint256 inviterIncome = 0;

//         bool isInviter = _invite.canInviter(inviter_);
...
//                         inviterIncome = price * (100 - _invite.inviteDiscountRate(_msgSender())) / 100;  
...
//                         }
//                     }else if(coinsType_ == 4){
//                         // matic
//                         price = priceInfo.mixedMaticAndKeyPrice.token1Price; 
//                         inviterIncome = price * (100 - _invite.inviteDiscountRate(_msgSender())) / 100;  
//                         require(msg.value >= price,"feeAmount not enough");
//                         if(isInviter){
//                             (success, ) = payable(inviter_).call{value: inviterIncome}("");
...
//                         //key
//                         price = priceInfo.mixedMaticAndKeyPrice.token2Price; 
//                         require(IERC20(_priceOfShorts[3].keyAddress).allowance(_msgSender(), address(this)) >= price,"019---allowance error!!!");
//                         if(isInviter) {
//                             inviterIncome = price * (100 - _invite.inviteDiscountRate(_msgSender())) / 100;
//                             IERC20(_coins[coinsType_]._coinAddress).transferFrom(_msgSender(),inviter_, inviterIncome);
//                             _invite.setInviterIncome(inviter_,0,inviterIncome);
//                             if((price * 70  / 100) - inviterIncome > 0){
//                                 IERC20(_coins[coinsType_]._coinAddress).transferFrom(_msgSender(),address(1),(price * 70  / 100) - inviterIncome);
//                             }
//                         }else{
//                             IERC20(_coins[coinsType_]._coinAddress).transferFrom(_msgSender(),address(1),price * 70 / 100);
//                         }  
//                         success = IERC20(_coins[coinsType_]._coinAddress).transferFrom(_msgSender(),_feeTo, price * 30 / 100);
...
//             priceInfo.mixedMaticAndKeyPrice.token1Price = 2**256 - 1;
//             priceInfo.mixedMaticAndKeyPrice.token2Price = 2**256 - 1;
//             return priceInfo;
//         }
//         uint256 nameLength = name_.lenOfChars();
...
//                 priceInfo.lowbPrice = 2**256 - 1;
//                 priceInfo.usdcPrice = 2**256 - 1;
//                 priceInfo.mixedMaticAndKeyPrice.token1Price = priceInfo.maticPrice / 2;
//                 priceInfo.mixedMaticAndKeyPrice.token2Price = priceInfo.maticPrice;
//             }else{
//                 priceInfo.maticPrice = 2**256 - 1;
//                 priceInfo.keyPrice = 2**256 - 1;
//                 priceInfo.lowbPrice = 2**256 - 1;
